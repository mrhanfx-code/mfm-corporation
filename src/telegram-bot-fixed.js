// MFM Corporation Telegram Bot - Fixed Enhanced AI System
...
class ConversationEngine {
  ...
  buildResponse(template, analysis, context, personality) {
    ...
    if (context && context.shortTerm && context.shortTerm.length > 0) {
  ...
  calculateConfidence(analysis, context) {
    ...
    if (context && context.shortTerm && context.shortTerm.length > 0) {
      confidence += Math.min(context.shortTerm.length * 0.02, 0.2);
    }
...
async function handleAdvancedMessage(message, env) {
  ...
  // FIXED: Move classification outside try block to fix scope issue
  const classification = classifyMessage(message.text || '', message, env);
  ...
  try {
    ...
    // FIXED: Use classification from outside try block
...
// FIXED: Add missing classifyMessage function
function classifyMessage(text, message, env) {
  const sensitiveKeywords = [
    'confidential', 'secret', 'private', 'internal', 'classified',
    'password', 'api key', 'token', 'credential', 'sensitive',
    'financial', 'legal', 'contract', 'agreement', 'nda'
  ];

  const lowerText = text.toLowerCase();
  const hasSensitiveContent = sensitiveKeywords.some(keyword => lowerText.includes(keyword));

  const hasAttachments = !!(message.photo || message.document || message.audio || message.video);
  const isLargeMessage = (text || '').length > 500;
...
// FIXED: Add handleTelegramRoute function
async function handleTelegramRoute(message, classification, env) {
  ...
  try {
    ...
    console.error('Error in handleTelegramRoute:', error);
    await sendMessage(chatId, 'Sorry, I encountered an error processing your request.', env);
    return new Response('Error in route handling', { status: 500 });
  }
}
...
async function sendViaEmail(message, classification, env) {
  ...
  try {
    const emailContent = `
      New Message from MFM AI Assistant
    ...
    // FIXED: Correct SendGrid API payload structure
    const sendGridPayload = {
      personalizations: [{
        to: [{ email: env.CEO_EMAIL }],
        subject: `MFM AI Message - ${classification.route.toUpperCase()} Route`
      }],
      from: { email: env.USER_EMAIL },
      content: [{
        type: 'text/plain',
        value: emailContent
      }]
    ...
    await sendMessage(chatId, '✅ Your message has been routed via email for secure handling.', env);
    return new Response('Email sent successfully');

  } catch (error) {
    console.error('Error sending email:', error);
    await sendMessage(chatId, '❌ Failed to send email. Please try again.', env);
