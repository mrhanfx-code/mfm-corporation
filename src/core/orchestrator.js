...
const SYSTEM_PROMPT = `You are REMY, the AI General Manager working directly for CEO Remy.
Your job is to understand the CEO's message and connect them with the right specialist agent.
...
- cto/backend-developer: Cloudflare Workers, D1 SQLite, KV, R2, API design, GitHub operations (create repo, list repos, push files)
...
GitHub routing override: Messages containing keywords "github", "repo", "repository", "create repo", "list repos" route to backend-developer
...
export async function routeMessage(message, userId, env) {
  ...
  // ── GitHub routing override: route to backend-developer ──
  const ghKeywords = ['github', 'repo', 'repository', 'create repo', 'list repos'];
  if (ghKeywords.some(k => lowerText.includes(k))) {
    await logDecision('orchestrator', text, 'GitHub operation routed to backend-developer', 'backend-developer', 1.0, env);
    syncRoutingDecision({ agent: 'backend-developer', taskType: 'GitHub operation', reasoning: 'GitHub keyword detected', confidence: 1.0 }, env).catch(() => {});
    
    const AgentClass = AGENT_MAP['backend-developer'];
    if (!AgentClass) return await handleDirect(text, userId, env);
    
    const contextCard = await buildContextCard('backend-developer', env);
    const agentOptions = { contextCard };
    const agent = new AgentClass();
    return await agent.run(text, userId, env, agentOptions);
  }
  ...
  try {
    ...
    if (env.KV && routing.agent !== 'research-agent') {
      ...
      if (review.score < 50) {
        ...
        if (fails >= 3) {
          ...
          return `Research Agent — Error Recovery\n${routing.agent} failed 3 times. Research Team intervened.\n\n${recovery}`;
        }
      } else {
    ...
    const header = `${routing.agent.replace(/-/g, ' ').toUpperCase()} (score: ${review.score}/100)\n\n`;
    ...
    return `System error: ${err.message}. Please try again.`;
  }
}
  ...
  switch (cmd) {
    ...
    case '/github': {
      ...
      if (sub === 'repos') {
        ...
        return `Repos (${r.repos.length}):\n` + r.repos.slice(0, 15).map(x => `• ${x.full_name} ${x.private ? '(private)' : '(public)'} — ${x.url}`).join('\n');
      }
      if (sub === 'create') {
    ...
    case '/memo':
      if (!args) return 'Usage: /memo [message to broadcast]';
    ...
    case '/remember': {
      if (!args) return 'Usage: /remember [content to remember]';
    ...
    case '/recall': {
      if (!args) return 'Usage: /recall [search query]';
      const results = await searchMemory(env, args, 5);
      if (results.length === 0) return 'No memories found.';
      const formatted = results.map(r => 
        `• ${r.content.slice(0, 100)}${r.content.length > 100 ? '...' : ''} ${r.pinned ? '(pinned)' : ''}`
      ).join('\n');
      return `Found ${results.length} memories:\n\n${formatted}`;
    }

    case '/pin': {
      if (!args) return 'Usage: /pin [memory ID]';
      const success = await pinMemory(env, args);
      return success ? 'Memory pinned.' : 'Failed to pin memory.';
    }

    case '/unpin': {
      if (!args) return 'Usage: /unpin [memory ID]';
      const success = await unpinMemory(env, args);
      return success ? 'Memory unpinned.' : 'Failed to unpin memory.';
    }

    case '/approve': {
      if (!env.KV) return 'KV not configured.';
      const raw = await env.KV.get(`pending:${userId}`);
      if (!raw) return 'No pending action found. It may have expired (1h TTL).';
      ...
      if (!AgentClass) return 'Agent not found for pending action.';
    ...
    case '/to': {
      ...
      if (!taskText) return 'Usage: /to [agent-name] [task]\nExample: /to finance-planner What is the Q3 forecast?';
      const AgentClass = AGENT_MAP[targetAgent];
      if (!AgentClass) return 'Unknown agent: "' + targetAgent + '". Type /help for the list.';
      const agent = new AgentClass();
      ...
      agent.finalizeScore(review.score, env).catch(() => {});
      const header = '[' + targetAgent.replace(/-/g, ' ').toUpperCase() + ']\n\n';
      return header + (review.improved_response || rawResponse);
    }

    case '/draft': {
      if (!args) return 'Usage: /draft [agent-name] [task]\nExample: /draft social-media-agent Post about our new product';
      const spaceIdx2 = args.indexOf(' ');
      const targetAgent = spaceIdx2 === -1 ? args : args.slice(0, spaceIdx2).trim();
      const taskText = spaceIdx2 === -1 ? '' : args.slice(spaceIdx2 + 1).trim();
      if (!taskText) return 'Usage: /draft [agent-name] [task]';
      const AgentClass = AGENT_MAP[targetAgent];
      if (!AgentClass) return 'Unknown agent: "' + targetAgent + '".';
      const agent = new AgentClass();
      const contextCard = await buildContextCard(targetAgent, env);
      ...
      return 'Draft — ' + targetAgent.replace(/-/g, ' ').toUpperCase() + '\n\n' + draft + '\n\n---\nReply /approve to execute or /edit [changes] to refine or /reject to cancel\nExpires in 1 hour';
    }

    case '/edit': {
      if (!env.KV) return 'KV not configured.';
      if (!args) return 'Usage: /edit [your changes]\nExample: /edit Make the tone more formal';
      const raw = await env.KV.get('pending:' + userId);
      if (!raw) return 'No pending draft found. Use /draft first.';
      const pending = JSON.parse(raw);
      const AgentClass = AGENT_MAP[pending.agentName];
      if (!AgentClass) return 'Agent not found.';
    ...
    case '/delegate': {
      if (!args) return `Usage: /delegate [agent-name] [task]\nExample: /delegate market-analyst What is our best LinkedIn strategy for Q3?`;
      const spIdx = args.indexOf(' ');
      if (spIdx === -1) return `Please include a task after the agent name.`;
      ...
      if (!DelegateClass) return `Unknown agent: ${targetAgent}\nSee /help for available agents.`;
    ...
    case '/panel': {
      if (!args) return `Usage: /panel [strategy|technical|content|innovation|operations|fullboard] [question]
Example: /panel strategy Should we enter the Johor market this quarter?`;
      ...
      if (!panelTask.trim()) return `Please include a question after the panel name.`;
      const panel = PANELS[panelKey];
      const panelAgents = panel.agentNames
        .map(n => PANEL_AGENT_REGISTRY[n]?.())
        .filter(Boolean);
...
function getToolsStatus(env) {
  const check = (key) => env[key] ? 'SET' : 'MISSING';
...
${env.KV ? 'SET' : 'MISSING'} KV Namespace (cache / approvals / circuit breaker)
${env.db ? 'SET' : 'MISSING'} D1 Database (memory / tasks / metrics)
${env['mfm-corporation-uploads'] ? 'SET' : 'MISSING'} R2 Bucket (file storage)
${env.TASK_QUEUE ? 'SET' : 'MISSING'} Queue (async tasks)
${env.AI ? 'SET' : 'MISSING'} Workers AI (image generation — Flux)
...
To fix missing: wrangler secret put KEY_NAME`;
}
...
async function getTeamReport(agentName, env) {
  if (!agentName) return 'Usage: /team [agent-name]\nExample: /team market-analyst';
...
async function broadcastMemo(memoText, userId, env) {
  ...
  return `Memo Drafted\n\n${draft}\n\nSend /memo [text] again after review to regenerate, or forward this directly.`;
}
...
async function getCircuitBreakerStatus(env) {
  ...
  const lines = await Promise.all(providers.map(async p => {
    ...
    const statusText = status.open ? 'OPEN' : 'OK';
    ...
    return `${statusText} ${p}${detail}`;
  }));
  return `Circuit Breaker Status\n\n${lines.join('\n')}\n\nOpen = provider temporarily bypassed. Resets after 60s.`;
}
